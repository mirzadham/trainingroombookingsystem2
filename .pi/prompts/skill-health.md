...
Run the skill health CLI in dashboard mode (script is project-local at `.pi/scripts/`):

```bash
node .pi/scripts/skills-health.js --dashboard
```

For a specific panel only:

```bash
node .pi/scripts/skills-health.js --dashboard --panel failures
```

For machine-readable output:

```bash
node .pi/scripts/skills-health.js --dashboard --json
```
