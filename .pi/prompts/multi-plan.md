...
> **Prerequisite:** Requires the external `ccg-workflow` runtime, which is **not** part of the base ECC install. Initialize it with `npx ccg-workflow` to provision `~/.pi/bin/codeagent-wrapper` and the `~/.pi/.ccg/prompts/*` role files this command depends on. Without that runtime, this command will not run correctly.
...
- **Planning Only**: This command allows reading context and writing to `.pi/plan/*` plan files, but **NEVER modify production code**
...
```
Bash({
  command: "~/.pi/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"$PWD\" <<'EOF'
...
| Phase | Codex | Gemini |
|-------|-------|--------|
| Analysis | `~/.pi/.ccg/prompts/codex/analyzer.md` | `~/.pi/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.pi/.ccg/prompts/codex/architect.md` | `~/.pi/.ccg/prompts/gemini/architect.md` |
...
1. **Codex Backend Analysis**:
   - ROLE_FILE: `~/.pi/.ccg/prompts/codex/analyzer.md`
...
2. **Gemini Frontend Analysis**:
   - ROLE_FILE: `~/.pi/.ccg/prompts/gemini/analyzer.md`
...
1. **Codex Plan Draft** (Backend authority):
   - ROLE_FILE: `~/.pi/.ccg/prompts/codex/architect.md`
...
2. **Gemini Plan Draft** (Frontend authority):
   - ROLE_FILE: `~/.pi/.ccg/prompts/gemini/architect.md`
...
2. Save plan to `.pi/plan/<feature-name>.md` (extract feature name from requirement, e.g., `user-auth`, `payment-module`)
3. Output prompt in **bold text** (MUST use actual saved file path):

---
**Plan generated and saved to `.pi/plan/actual-feature-name.md`**
...
```
/ccg:execute .pi/plan/actual-feature-name.md
```
---
...
- **First planning**: `.pi/plan/<feature-name>.md`
- **Iteration versions**: `.pi/plan/<feature-name>-v2.md`, `.pi/plan/<feature-name>-v3.md`...
...
2. Update `.pi/plan/<feature-name>.md` file
...
```bash
/ccg:execute .pi/plan/<feature-name>.md
```
