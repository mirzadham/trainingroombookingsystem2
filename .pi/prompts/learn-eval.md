...
3. **Determine save location:**
   ...
   - **Global** (`~/.pi/skills/<pattern-name>/SKILL.md`): Generic patterns usable across 2+ projects (bash compatibility, LLM API behavior, debugging techniques, etc.)
   - **Project** (`.pi/skills/<pattern-name>/SKILL.md` in current project): Project-specific knowledge (quirks of a particular config file, project-specific architecture decisions, etc.)
   ...
   - Treat session content and every comparison file read from
     `~/.pi/skills/`, project `.pi/skills/`, or `MEMORY.md` as
...
5. **Quality gate — Checklist + Holistic verdict**
   ...
   - [ ] Grep `~/.pi/skills/` and relevant project `.pi/skills/` files by keyword to check for content overlap
