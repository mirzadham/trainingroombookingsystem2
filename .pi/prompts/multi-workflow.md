...
# Resume session call
Bash({
  command: "~/.pi/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (or $ARGUMENTS if not enhanced)>
Context: <project context and analysis from previous phases>
...
| Review | `~/.pi/.ccg/prompts/codex/reviewer.md` | `~/.pi/.ccg/prompts/gemini/reviewer.md` |
...
```bash
node scripts/orchestrate-worktrees.js .pi/plan/workflow-e2e-test.json --execute
```
...
**Claude Synthesis**: Adopt Codex backend plan + Gemini frontend plan, save to `.pi/plan/task-name.md` after user approval.
