...
```bash
mkdir -p .pi/PRPs/reports
```

Write report to `.pi/PRPs/reports/{plan-name}-report.md`:
...
```bash
mkdir -p .pi/PRPs/plans/completed
mv "$ARGUMENTS" .pi/PRPs/plans/completed/
```
...
### Artifacts
- Report: `.pi/PRPs/reports/{name}-report.md`
- Archived Plan: `.pi/PRPs/plans/completed/{name}.plan.md`
