---
description: Generate a local cost report from the cost-tracker metrics log.
argument-hint: [csv]
---

# Cost Report

Summarize local spend by day, model, and session from the metrics
log that ECC's `stop:cost-tracker` hook writes (set one up under `.pi/hooks/` if not present).

## Where the data lives

The tracker appends one JSON object per session-stop to
`~/.pi/metrics/costs.jsonl`. Each row is a **cumulative snapshot for that
...
1. Check that `~/.pi/metrics/costs.jsonl` exists. If it does not, tell the
...
const f=path.join(os.homedir(),".pi","metrics","costs.jsonl");
