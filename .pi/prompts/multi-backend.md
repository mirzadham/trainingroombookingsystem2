...
# New session call
Bash({
  command: "~/.pi/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend codex - \"$PWD\" <<'EOF'
...
# Resume session call
Bash({
  command: "~/.pi/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend codex resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (or $ARGUMENTS if not enhanced)>
Context: <project context and analysis from previous phases>
...
| Phase | Codex |
|-------|-------|
| Analysis | `~/.pi/.ccg/prompts/codex/analyzer.md` |
| Planning | `~/.pi/.ccg/prompts/codex/architect.md` |
| Review | `~/.pi/.ccg/prompts/codex/reviewer.md` |
