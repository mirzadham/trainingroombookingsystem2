---
description: Save current session state to a dated file in ~/.pi/session-data/ so work can be resumed in a future session with full context.
---
...
```bash
mkdir -p ~/.pi/session-data
```

### Step 3: Write the session file

Create `~/.pi/session-data/YYYY-MM-DD-<short-id>-session.tmp`, using today's actual date and a short-id that satisfies the rules enforced by `SESSION_FILENAME_REGEX` in `session-manager.js`:
...
- Use the canonical global session store: `~/.pi/session-data/`
