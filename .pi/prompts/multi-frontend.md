...
# New session call
Bash({
  command: "~/.pi/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend gemini --gemini-model gemini-3-pro-preview - \"$PWD\" <<'EOF'
...
# Resume session call
Bash({
  command: "~/.pi/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend gemini --gemini-model gemini-3-pro-preview resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (or $ARGUMENTS if not enhanced)>
Context: <project context and analysis from previous phases>
...
| Phase | Gemini |
|-------|--------|
| Analysis | `~/.pi/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.pi/.ccg/prompts/gemini/architect.md` |
| Review | `~/.pi/.ccg/prompts/gemini/reviewer.md` |
...
Claude synthesizes plan, save to `.pi/plan/task-name.md` after user approval.
