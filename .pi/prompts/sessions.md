...
Manage Claude Code session history - list, load, alias, and edit sessions stored in `~/.pi/session-data/` with legacy reads from `~/.pi/sessions/`.
...
```bash
node -e "
const _r = require('path').resolve('.pi');
...
- Sessions are stored as markdown files in `~/.pi/session-data/` with legacy reads from `~/.pi/sessions/`
- Aliases are stored in `~/.pi/session-aliases.json`
