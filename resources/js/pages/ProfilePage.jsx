...
import { User, Lock, Mail, Phone, Building, ShieldCheck, CheckCircle2, AlertCircle, CalendarDays, Copy, RefreshCw, Check } from 'lucide-react';
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 min-h-[calc(100vh-20rem)]">
            ...
            <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="w-full md:w-64 shrink-0 space-y-2">
                    ...
                    <button
                        onClick={() => { setActiveTab('calendar'); setSuccessMessage(''); setErrorMessage(''); setValidationErrors({}); }}
                        className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-200 cursor-pointer ${
                            activeTab === 'calendar'
                                ? 'bg-mimos-50 text-mimos-700 shadow-sm border border-mimos-100/50'
                                : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100/70 border border-transparent'
                ...
                <div className="flex-1 bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-8 shadow-sm">
                    ...
                    {activeTab === 'calendar' && <CalendarSubscriptionTab />}
...
/**
 * Calendar Sync tab — private iCal subscription feed for the user's
 * approved bookings (Outlook / Google / Apple Calendar).
 */
function CalendarSubscriptionTab() {
    const queryClient = useQueryClient();
    const [copied, setCopied] = useState(false);

    const { data, isLoading } = useQuery({
        queryKey: ['calendar-subscription'],
        queryFn: () => api.getCalendarSubscription(),
    });

    const regenerateMutation = useMutation({
        mutationFn: () => api.regenerateCalendarSubscription(),
        onSuccess: (res) => {
            queryClient.setQueryData(['calendar-subscription'], res);
            setCopied(false);
        },
    });
    ...
    const copyToClipboard = async (text) => {
        try {
            ...
            // Clipboard API unavailable — fall back to a prompt for manual copy.
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                    See your approved bookings in Outlook, Google Calendar or Apple Calendar — updates automatically.
                </p>
            </div>
            ...
            {isLoading && (
                <div className="flex items-center justify-center py-10">
                    <RefreshCw className="w-5 h-5 text-mimos-500 animate-spin" />
                </div>
            )}

            {!isLoading && data && (
                <div className="space-y-5">
                    {/* Webcal link (Outlook / Apple) */}
                    <div>
                        <label className="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wider">
                            Subscribe Link (Outlook / Apple Calendar)
                        </label>
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 font-mono focus:outline-none focus:ring-2 focus:ring-mimos-500/50 truncate"
                            />
                            <button
                                onClick={() => copyToClipboard(data.feed_url)}
                                className="flex items-center gap-1.5 px-4 py-2.5 bg-mimos-500 hover:bg-mimos-600 text-white text-xs font-semibold rounded-xl transition cursor-pointer shrink-0"
                            >
                        ...
                        <p className="text-[11px] text-slate-400 mt-1.5">
                            In Outlook: <span className="font-mono">File → Account Settings → Internet Calendars → New</span>. In Apple Calendar: File → New Calendar Subscription.
                        </p>
                    </div>

                    {/* HTTPS link (Google Calendar) */}
                    <div>
                        <label className="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wider">
                        ...
                        <div className="flex gap-2">
                            <input
                                type="text"
                                readOnly
                                value={data.https_url}
                                onFocus={(e) => e.target.select()}
                                className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 font-mono focus:outline-none focus:ring-2 focus:ring-mimos-500/50 truncate"
                            />
                            <button
                                onClick={() => copyToClipboard(data.https_url)}
                                className="flex items-center gap-1.5 px-4 py-2.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 text-xs font-semibold rounded-xl transition cursor-pointer shrink-0"
                            >
                        ...
                        <p className="text-[11px] text-slate-400 mt-1.5">
                            In Google Calendar: <span className="font-mono">Settings → Add calendar → From URL</span>.
                        </p>
                    </div>

                    {/* Security note + regenerate */}
                    <div className="p-4 rounded-xl bg-amber-50/60 border border-amber-100">
                        <div className="flex items-start gap-2.5">
                            <AlertCircle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                            <div className="text-xs text-amber-800 leading-relaxed">
                                <span className="font-semibold">This link is a secret.</span> Anyone with it can view your
                                booking schedule. If you think it has been shared, regenerate it — old links stop working immediately.
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-slate-100 flex justify-end">
                        <Button
                            onClick={() => {
                                if (window.confirm('Regenerating this link will break all existing calendar subscriptions. Continue?')) {
